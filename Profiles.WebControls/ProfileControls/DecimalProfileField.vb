...
Namespace Profiles.WebControls.ProfileControls
    ''' <summary>
    ''' This control encapsulates the user interface required to present a decimal field value to the user and enable them to edit it.
    ''' </summary>
    Public Class DecimalProfileField
        Inherits WebControl
        Implements INamingContainer, IProfileField

        Private mTextField As TextBox
        Private mField As FieldMetadataWrapper
        Private mValidator As Sample.Web.UI.Compatibility.RegularExpressionValidator
        ...
        Protected Overrides Sub CreateChildControls()
            ...
            mTextField.ID = "decimalBox"
            mTextField.Style.Add("width", "120px")
            mTextField.MaxLength = 17
            ...
            mValidator = New Sample.Web.UI.Compatibility.RegularExpressionValidator
            mValidator.ID = "rvalDecimal"
            mValidator.ControlToValidate = mTextField.ID
            mValidator.Text = "*"
            mValidator.ErrorMessage = "Please enter a decimal with up to 2 decimal places"
            mValidator.ToolTip = "Please enter a decimal with up to 2 decimal places"
            'this regex means: a character 1-9 followed by 0-13 more digits, or a zero, 
            'optionally followed by a decimal point and 1-2 more digits
            mValidator.ValidationExpression = "(([1-9]\d{0,13})|0)(\.\d{1,2})?"
            mValidator.EnableClientScript = True

            Controls.AddAt(0, mValidator)
        ...
        Public Sub Save(ByVal value As FieldValueBase) Implements IProfileField.Save
            ...
            If Not mField.IsReadOnly Then
                If mTextField.Text.Length = 0 Then
                    DirectCast(value, DecimalFieldValue).FieldValue = Nothing
                Else
                    Try
                        DirectCast(value, DecimalFieldValue).FieldValue = System.Convert.ToDecimal(mTextField.Text)
                    Catch ex As Exception
                        Throw New InvalidOperationException("Cannot save as value is not a valid decimal")
                    End Try
