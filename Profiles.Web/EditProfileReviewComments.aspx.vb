...
#Region "Review comments"
    ...
    Public Function FormatSubject(ByVal subject As String) As String
        ...
        If String.IsNullOrEmpty(subject) Then subject = ""
    ...
    Private Sub SwitchToReviewCommentReadOnlyView(ByVal readOnlyReviewComment As ReviewComment)
        ...
        'litComment.Text = String.Format("<p style=""border-style:solid;border-width:1px;background-color:white;width:95%;height:135px;overflow-y:scroll;font-size:80%""><span>{0}</span></p>", readOnlyReviewComment.CommentText.Replace(ControlChars.NewLine, "<br/>"))
        litCommentSubject.Text = readOnlyReviewComment.CommentSubject.Replace(ControlChars.NewLine, "<br/>").SanitizeHtml
        litCommentReference.Text = readOnlyReviewComment.CommentReference.Replace(ControlChars.NewLine, "<br/>").SanitizeHtml
        litComment.Text = readOnlyReviewComment.CommentText.Replace(ControlChars.NewLine, "<br/>").SanitizeHtml()
    ...
    Private Sub SwitchToReviewCommentEditView(ByVal editReviewComment As ReviewComment)
        ...
        spellUserGuidance.ControlToCheck = $"{txtComment.ClientID}_ifr"
...
#End Region
    ...
    Private Sub EditReviewComments_PreRender(sender As Object, e As EventArgs) Handles Me.PreRender
        RegisterTinyMCE($"#{txtComment.ClientID}", TinyMCEHelper.TinyMCEEditorType.LinkOnlyEditor, "200", lblCommentComment.Text)
    End Sub

    Private Sub EditProfileReviewComments_Load(sender As Object, e As EventArgs) Handles Me.Load

        If Page.IsPostBack Then
            HandleCustomPostBackFromTabManager()
        End If

        SectionPagination.TargetUrl = "EditProfileReviewComments.aspx?ProfileVersionId=" & mProfileVersionInfo.Id.ToString & "&ProfileSectionId={0}"
    ...
    Private Sub bottomPaginator_PageChanged(sender As Object, e As PageChangedEvent) Handles bottomPaginator.PageChanged
        ...
        grdReviewComments.PageIndex = bottomPaginator.CurrentPage - 1
        RefreshPage()
    ...
    Private Sub RefreshPage()
        grdReviewComments.DataBind()
        bottomPaginator.TotalPages = grdReviewComments.PageCount
