...
Public Class SpeciesListLabelUpdatedEventArgs
	Inherits EventArgs

	Public ReadOnly Property ListLabel As String

	Public Sub New(list As String)
		Me.ListLabel = list
	End Sub
End Class
...
Partial Class SpeciesSelector
	...
	Public Event SpeciesListUpdated As EventHandler(Of SpeciesListLabelUpdatedEventArgs)
	Public ShowLabel As Boolean = True 
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
		...
		lblAffectedSpecies.Visible = Me.ShowLabel

		Dim scriptFile = "Javascript/species-selector.js"

		ScriptManager.RegisterClientScriptInclude(Me, Me.GetType, "scriptFile", scriptFile)

		Dim labelClientId = If(lblAffectedSpecies.Visible, lblAffectedSpecies.ClientID, LabelId)

        ScriptManager.RegisterStartupScript(Me, Me.GetType(),
                                            "speciesSelector",
                                            $"<script>window.speciesBehaviour = new SpeciesSelectorBehaviour('{labelClientId}')</script>",
                                            False)
		...
		If speciesList.Count > 0 Then
			speciesList.Sort(Function(left As String, right As String)
								 Return left.StripHtmlTags.CompareTo(right.StripHtmlTags)
							 End Function)
		...
		LabelChanged(lblAffectedSpecies.Text)

	End Sub

	Public Function GetSpeciesValue() As String
		Return lblAffectedSpecies.Text
	End Function
	...
	Private Sub AddTreeNodesForDisease(
		ByVal speciesList As SpeciesInfoList,
		ByVal profiledSpeciesNodeList As RadTreeNodeCollection,
		ByVal profiledParentChecked As Boolean
	)
		Dim i As Integer = 0
		For Each speciesChild As SpeciesInfo In speciesList
			'Cycle through the species list
			...
			'Recursively call AddTreeNodes function for any child nodes
			AddTreeNodesForDisease(
				speciesChild.ChildList,
				profiledSpeciesNode.Nodes,
				profiledSpeciesNode.Checked
			)
	...
	Private Sub LabelChanged(list As String)
		RaiseEvent SpeciesListUpdated(Me, New SpeciesListLabelUpdatedEventArgs(list))
	End Sub
