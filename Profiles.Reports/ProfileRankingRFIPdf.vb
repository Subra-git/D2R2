...
Public Class ProfileRankingReportRFIPdf
    Inherits PdfBase
    ...
    Public mTitle As String = "Disease ranking report. "

    Private Function GetBlankLine() As XhtmlParagraph
        Dim blankLine As New XhtmlParagraph
        blankLine.Text = "<span style='font-size: 1'><br /></span>"
        Return blankLine
    End Function

    Private Function GetBlankRow() As Row
        Dim row As New Row
    ...
    Protected Overrides Function CreateFrontPage() As TallComponents.PDF.Layout.Section
        ...
        If (ReportType.ToString() <> "") Then
            mTitle = "Disease ranking report for " + If(ReportType.ToString() = ProfileRankingReportType.Fish.ToString(), "Aquatic", If(ReportType.ToString() = ProfileRankingReportType.Terrestrial.ToString(), "Terrestrial", If(ReportType.ToString() = ProfileRankingReportType.All.ToString(), mReportFilterType, ""))) + "."
        End If
    ...
    Protected Overrides Sub BuildDocumentBody()
        ...
        For count As Integer = 0 To graphParmsList.Count - 1 Step 2
            ...
            filteredList1 = mRankProfileList.GetAllSortedList( _
            ...
            Dim FullListOfLists As List(Of List(Of RankProfileInfo)) = GetChunkedDataLists(filteredList1, 49)

            Dim title1 As String

            title1 = graphParmsList(count).Title

            Dim currentFullList As List(Of RankProfileInfo) = Nothing

            If FullListOfLists.Count > 0 Then
                currentFullList = FullListOfLists(0)
            Else
                currentFullList = New List(Of RankProfileInfo)
            End If


            Dim chartText As StringBuilder = New StringBuilder()
            chartText.Append("Graphical report showing at most the top 50 ranked profiles for each RFI. ")
            chartText.Append("Each disease will only have (at most) one entry regardless of how many species have been considered in the profile. For multiple species disease profiles the species that will appear in this graph is the one with the largest sum of the PMR score and RFI importance. ")
            chartText.Append("Each graph is designed to show the post mitigation risk score (PMR, coloured black) next to the axis and with the ranking for the specific RFI shown next to it. ")
            chartText.Append("Overall potential disease impact for that RFI is the combination of the PMR score and the RFI ranking, with the highest impact at the top of the graph. ")
            chartText.Append("Diseases that do not achieve a ranking for this RFI will not appear in this graph. Note: * = Disease profiles awaiting validation. EXOTIC DISEASES APPEAR IN CAPITALS and endemic diseases in standard case format.")
            ...
            If Not graphParmsList(count).Category <> RankCategory.International_Trade Then
                ...
                sb.Append("Therefore the graph of endemic diseases does not contain many diseases. Therefore the international trade graph contains very few (if any) endemic diseases.")
        ...
        Dim filteredList3 As List(Of RankProfileInfo) = mRankProfileList.GetAllSortedList(RankCategory.Risk, False)
        Dim filteredList4 As List(Of RankProfileInfo) = mRankProfileList.GetAllSortedList(RankCategory.Total_Including_Risk, False)
        ...
        For Each item As List(Of RankProfileInfo) In listOfLists1
            If count2 = 1 Then
                PdfSection.Paragraphs.Add(GetChartTable(RankCategory.Total, title3, GetTotalChartParagraphs(), item, False, 0, yMax1))
            Else
                PdfSection.Paragraphs.Add(GetChartTable(RankCategory.Total, title3 & " continued...", GetTotalChartParagraphs(), item, True, 0, yMax1))
            End If
            count2 = count2 + 1
        Next
        ...
        For Each item As List(Of RankProfileInfo) In listOfLists2
            If count2 = 1 Then
                PdfSection.Paragraphs.Add(GetChartTable(RankCategory.Total_Including_Risk, title4, GetTotalPlusRiskChartParagraphs(), item, True, 0, yMax2))
            Else
                PdfSection.Paragraphs.Add(GetChartTable(RankCategory.Total_Including_Risk, title4 & " continued...", GetTotalPlusRiskChartParagraphs(), item, True, 0, yMax2))
            End If
            count2 = count2 + 1
        Next
    ...
    Private Function GetChartTable(ByVal category As RankCategory, _
                                   ...
                                   ByVal yMin As Double, _
                                   ByVal yMax As Double) As Paragraphs.Table
        Dim frag As Fragment = New Fragment(text.StripHtmlTags)
        frag.FontSize = 9
    ...
    Private Function GetCategoryBackColor(ByVal category As RankCategory) As Color
        ...
        Select Case category
            ...
            Case RankCategory.Wider_Society
                Return Color.FromArgb(0, 178, 238) 'Blue
            Case RankCategory.Animal_Welfare
                Return Color.FromArgb(61, 145, 64) 'Green
            Case RankCategory.Public_Health
                Return Color.FromArgb(238, 58, 140) 'Pink
    ...
    Private Function GetChartImage(ByVal title As String, _
        ...
        Select Case category
            ...
            Case RankCategory.Public_Health
                AddChartSeries(chart, "Post mitigation risk", "FullTitle", "PmrRank", RankCategory.Risk)
                AddChartSeries(chart, "Public health", "FullTitle", "PublicHealthPmrRank", RankCategory.Public_Health)
                AddChartSeries(chart, "Welfare", "FullTitle", "AnimalWellfarePmrRank", RankCategory.Animal_Welfare)
                AddChartSeries(chart, "Wider society", "FullTitle", "WiderSocietyPmrRank", RankCategory.Wider_Society)
                AddChartSeries(chart, "International trade", "FullTitle", "InternationalTradePmrRank", RankCategory.International_Trade)
            Case RankCategory.Animal_Welfare
                AddChartSeries(chart, "Post mitigation risk", "FullTitle", "PmrRank", RankCategory.Risk)
                AddChartSeries(chart, "Welfare", "FullTitle", "AnimalWellfarePmrRank", RankCategory.Animal_Welfare)
                AddChartSeries(chart, "Wider society", "FullTitle", "WiderSocietyPmrRank", RankCategory.Wider_Society)
    ...
    Protected Function GetTotalChartParagraphs() As List(Of TextParagraph)
        ...
        totalText.Append("Graphical report showing all the rankings for all disease profiles with multiple entries for each profiled species considered within each disease profile. ")
        totalText.Append("This graph is designed to show the post mitigation risk score (PMR, coloured black) next to the axis and with the ranking for every relevant RFI shown adjacent. ")
        totalText.Append("The highest PMR score appears at the top of the graph. ")
        totalText.Append("Note: * = Disease profiles awaiting validation. EXOTIC DISEASES APPEAR IN CAPITALS and endemic diseases in standard case format.")
    ...
    Protected Function GetTotalPlusRiskChartParagraphs() As List(Of TextParagraph)
        ...
        totalPlusRiskText.Append("Graphical report showing all the rankings for all disease profiles with multiple entries for each profiled species considered within each disease profile. ")
        totalPlusRiskText.Append("This graph is designed to show the post mitigation risk score (PMR, coloured black) next to the axis and with the ranking for every relevant RFI shown adjacent. ")
        totalPlusRiskText.Append("The disease / species combination with the largest overall combination of PMR score added to ranked position for the RFIs appears at the top of the graph. ")
        totalPlusRiskText.Append("However this total score should not be used for resource allocation decisions - the rankings per RFI are the more appropriate tool to use to support decision making. ")
        totalPlusRiskText.Append("Note: * = Disease profiles awaiting validation. EXOTIC DISEASES APPEAR IN CAPITALS and endemic diseases in standard case format.")
    ...
    Private Class GraphParameters
        ...
        Public ReadOnly Property Title() As String
            Get
                ...
                If mIsExotic Then
                    sb.Append("Diseases ranked according to potential impact on ")
                Else
                    sb.Append("Diseases ranked according to potential impact on ")
                End If
